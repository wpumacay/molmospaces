...
def from_pd_joint_pos_to_ee(
    ...
    """
    Convert pd_joint_pos actions to pd_ee_delta_pose actions.
    
    This version correctly handles robot base rotation by computing delta pose
    relative to the robot base frame (ee_pose_at_base), which is what the
    PDEEPoseController actually uses internally.
    
    The key insight is that PDEEPoseController uses ee_pose_at_base (EE pose
    relative to robot base) for all computations, so we must compute target
    EE pose in the same frame using forward kinematics.
    """
    n = len(ori_actions)
    if pbar is not None:
        pbar.reset(total=n)
    ...
    for t in range(n):
        ...
        # Compute TARGET EE pose at base using FK with target qpos
        # FK computes pose relative to robot base (local frame), which is
        # exactly what ee_pose_at_base represents
        ...
        target_ee_pose_at_base = Pose.create(
        ...
        for _ in range(4):
            if target_controller_is_delta:
                # Get CURRENT EE pose at base from the env's controller
                if use_target:
                    current_ee_pose_at_base = arm_controller._target_pose
                else:
                    current_ee_pose_at_base = arm_controller.ee_pose_at_base
                
                # Compute delta position in base frame
                delta_position = (
                    target_ee_pose_at_base.p
                    - current_ee_pose_at_base.p
                )
                
                # Compute delta rotation
                # In root_aligned_body_rotation frame, the controller computes:
                #   target_q = delta_q * current_q
                # So to get delta_q that produces target_q from current_q:
                #   delta_q = target_q * current_q^-1
                # But ManiSkill's original code uses: current * target^-1
                # This gives the INVERSE of what we want, which then gets negated
                # by compact_axis_angle_from_quaternion when the quaternion has w < 0
                if "root_aligned_body_rotation" in arm_controller.config.frame:
                    # Both poses are in base frame now (ee_pose_at_base)
                    # Use sapien Pose multiplication: (current * target^-1) 
                    delta_q = (current_ee_pose_at_base.sp * target_ee_pose_at_base.sp.inv()).q
                else:
                    delta_q = np.array([1, 0, 0, 0])
