...
import molmo_spaces_maniskill.molmoact2.tasks
from mani_skill.envs.utils.system.backend import CPU_SIM_BACKENDS
from mani_skill.trajectory import utils as trajectory_utils
from mani_skill.trajectory.merge_trajectory import merge_trajectories
from molmo_spaces_maniskill.teleop.utils import action_conversion
from mani_skill.utils import common, io_utils, wrappers
from mani_skill.utils.logging_utils import logger
from mani_skill.utils.wrappers.flatten import FlattenActionSpaceWrapper
from mani_skill.utils.wrappers.record import RecordEpisode

from molmo_spaces_maniskill.teleop.wrappers.record_with_freq import RecordEpisodeWithFreq


@dataclass
class Args:
    traj_path: str = "/home/shuo/research/molmospaces/molmo_spaces_maniskill/demos/DroidKitchenOpenDrawerMem-v1/gello_teleop_20260310_144439/trajectory.h5"
    ...
    Use pd_ee_target_delta_pose (use_target=True) for stable conversion with rotated robot bases.
    ...
    save_action_plot: bool = False
    """Whether to save action plots for each episode"""
...
def save_action_plot(actions: np.ndarray, output_path: str, episode_id: int, joint_names: list[str] = None):
    """
    Save a plot of actions over time for an episode.
    
    Args:
        actions: (T, action_dim) array of actions
        output_path: Directory to save the plot
        episode_id: Episode ID for the filename
        joint_names: Optional list of joint names for labels
    """
    if len(actions) == 0:
        return
    ...
    # Determine layout based on action dimension
    if action_dim <= 6:
        n_cols = 2
    elif action_dim <= 12:
        n_cols = 3
    ...
    for i in range(action_dim):
        ...
        ax.plot(timesteps, actions[:, i], linewidth=1.5, color='C0')
    ...
    for i in range(action_dim, n_rows * n_cols):
        row, col = i // n_cols, i % n_cols
        axes[row, col].set_visible(False)
    
    plt.suptitle(f"Episode {episode_id} Actions", fontsize=12, fontweight='bold')
    plt.tight_layout()
    
    os.makedirs(output_path, exist_ok=True)
    plot_path = os.path.join(output_path, f"episode_{episode_id}_actions.png")
    plt.savefig(plot_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    tqdm.write(f"Saved action plot to {plot_path}")
...
def replay_cpu_sim(
    args: Args, env: RecordEpisode, ori_env, pbar, episodes, trajectories, output_dir: str = None
):
    successful_replays = 0
    for episode in episodes:
        ...
        for _ in range(args.max_retry + 1):
            ...
            if success or args.allow_failure:
                ...
                if args.save_action_plot and output_dir is not None:
                    save_action_plot(ori_actions, output_dir, episode_id)
                break
            else:
                if args.verbose:
                    print("info", info)
...
def _main(
    ...
    # Save videos to a 'replay' subfolder under the trajectory directory
    traj_dir = os.path.dirname(traj_path)
    output_dir = os.path.join(traj_dir, "replay")
    ...
    if use_cpu_backend:
        ...
        if args.target_control_mode is not None:
            ...
            # Unwrap to get the base ManiSkill env - action_conversion needs .agent attribute
            # ManiSkill's TimeLimitWrapper doesn't proxy .agent, so we must unwrap
            ori_env = ori_env.unwrapped
        else:
            ori_env = None
    else:
        pass
    ...
    if use_cpu_backend:
        ...
        replay_result = replay_cpu_sim(
            args, env, ori_env, pbar, [episodes[index] for index in inds], ori_h5_file, output_dir
        )
    else:
