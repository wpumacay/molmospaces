...
@register_env("DroidAi2LabOpenMicrowave-v1")
class DroidAi2LabOpenMicrowaveEnv(MolmoSpacesEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    def _load_scene(self, options: dict):
        ...
        # Find and store reference to microwave articulation for success checking
        self.microwave = None
        for name, articulation in self._env_articulations.items():
            if "microwave" in name.lower():
                self.microwave = articulation
                break
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # Reset microwave door to slightly open position (-0.05)
            # Door_01_joint is at index 2 based on the screenshot
            if self.microwave is not None:
                qpos = self.microwave.get_qpos()
                # qpos shape is (num_envs, num_joints), use [..., 2] for the door joint
                qpos[..., 2] = -0.05  # Door_01_joint initial position
                self.microwave.set_qpos(qpos)



    def evaluate(self):
        # Success is achieved when microwave Door_01 is open < -0.3 radians
        # Door_01_joint is at index 2 in the microwave articulation qpos
        if self.microwave is not None:
            microwave_qpos = self.microwave.get_qpos()
            # Door_01_joint is index 2 (based on the screenshot: Button_01=0, Disc_01=1, Door_01=2)
            door_01_angle = microwave_qpos[..., 2]  # Shape: (num_envs,) or scalar
            ...
            # Success when door angle < -0.3 (more negative = more open)
