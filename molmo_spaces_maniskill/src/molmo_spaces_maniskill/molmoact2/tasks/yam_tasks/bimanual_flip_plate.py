...
@register_env("BimanualYAMLFlipPlate-v1", max_episode_steps=200)
class BimanualYAMLFlipPlateEnv(BaseEnv):
    """
    ...
    pot_spawn_height = 0.05
    horizon_thresh_deg = 20.0
    table_height_thresh = 0.03
    ...
    def _load_scene(self, options: dict):
        ...
        plate_loader = MjcfAssetActorLoader(self.scene)
        plate_builder = plate_loader.load_from_xml(plate_xml, floating_base=True)
        try:
            self.plate = plate_builder.build(name="plate")
        except TypeError:
            plate_builder.set_name("plate")
            self.plate = plate_builder.build()
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            plate_xyz[:, 0] = -0.30 + (torch.rand((b,)) - 0.5) * 0.04
            plate_xyz[:, 1] = (torch.rand((b,)) - 0.5) * 0.08
            plate_xyz[:, 2] = self.pot_spawn_height
            plate_q = torch.zeros((b, 4))
            plate_q[:, :] = torch.tensor(self.pot_spawn_quat, device=self.device, dtype=plate_xyz.dtype)
            self.plate.set_pose(Pose.create_from_pq(plate_xyz, plate_q))
    
    def _get_obs_extra(self, info: dict) -> Dict[str, Any]:
        """Get extra observations."""
        return dict(plate_pose=self.plate.pose.raw_pose)
    
    def evaluate(self) -> Dict[str, Any]:
        """Success when the plate is flipped upside down and resting back on the table."""
        ...
        plate_height = self.plate.pose.p[:, 2]
        on_table = plate_height <= self.table_height_thresh

        plate_tf = self.plate.pose.to_transformation_matrix()
        upright_alignment = (
            self.pot_upright_axis_sign * plate_tf[..., 2, self.pot_upright_axis_idx]
        )
        flipped = upright_alignment <= -np.cos(np.deg2rad(self.horizon_thresh_deg))

        success = flipped & on_table #& released
        return {
            "success": success,
