"""
Pytest configuration for data generation curobo tests.
...
def collect_profilers(request):
    ...
    # Collect profilers from config fixtures - add config fixture names as needed
    for fixture_name in request.fixturenames:
        if "config" in fixture_name:  # Match any fixture with 'config' in the name
            try:
...
def pytest_sessionfinish(session, exitstatus):
    ...
    # Also save to file
    output_file = Path("mlspaces_tests/data_generation_curobo") / "profiling_results.txt"
